...
LOOP is presented as a visual environment to teach OOP using a reduced set of language constructions and a prototype approach to create objects.  It presents the main concepts of object, message and reference in a specialized tool with a visual representation of the object environment.

ObjectKarel\cite{Findl02} presents a visual tool based on the abstraction of robots to teach OOP, using a map where the robots-the objets- move when messages are sent to them.  LOOP does not center on a specific abstraction like a robot: it allows the student to create any other abstraction.  Also, the visual approachs differ in what they want to show.  LOOP is mainly oriented to show an object environment, emphasizing the relations between the objets and their identity.  The state changes seen in ObjectKarel through a map, are seen in LOOP in the object references.

Projects like Scratch\cite{} and Etoys\cite{Kay} on the other way, are aimed to teach the basics of programming to children, using visual objects and scripts to play with them.  LOOP differs from these project because it is mainly oriented to teach professional programming to people who wants to enter in the software industry.

LOOP's prototype approach is mainly based on the ideas of Self \cite{Unga07} and Javascript \cite{Wilk02}, differing in some crucial points.  First, LOOP's model is more restrictive than the existing in Self, allowing only one parent for each object.  Second, the idea of parent slots/prototypes in LOOP is completely handled by the tool, whithout letting the student to manage them.  This way the transition from single prototyping to single inheritance is easier for the student when he faces a programming language like Java or Smalltalk, and it also let's him to focus on the concepts the tools wants to show -object, message, relations, polymorphism- instead of more complex ones -classes, inheritance, traits, mixins.
...
\bibitem[Findler, Clements, Flanagan, Flatt, Krishnamurthi, Steckler and Felleisen(2002)]{Findl02}
...
\bibitem[Satratzemi et~al.(2003)Satratzemi, Xinogalos and Dagdilelis]{Satra03}
...
http://portal.acm.org/citation.cfm?id=1858609
